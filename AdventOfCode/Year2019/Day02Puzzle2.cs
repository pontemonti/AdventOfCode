...
namespace Pontemonti.AdventOfCode.Year2019
{
    ...
    /// - you'll probably use it again. Real Intcode computers support many more features than 
    /// your new one, but we'll let you know what they are as you need them."
    /// 
    ...
    /// initializing memory to the program's values.
    /// A position in memory is called an address (for example, the first value in memory is at 
    /// "address 0").
    ...
    /// just like before.
    /// In this program, the value placed in address 1 is called the noun, and the value placed in 
    ...
    /// </summary>
    public class Day2Puzzle2 : IPuzzle
    {
        public void Solve()
        {
            int result = CalculateResult();
            Console.WriteLine($"100 * noun + verb = {result}");
        }

        public static int CalculateResult()
        {
            (int noun, int verb) = FindNounAndVerb();
            int result = 100 * noun + verb;
            return result;
        }

        public static (int, int) FindNounAndVerb()
        {
        ...
        {
            int[] integers = InputHelper.ReadIntegerCommaList(Day02Puzzle1.input).ToArray();
            integers[1] = noun;
            integers[2] = verb;
