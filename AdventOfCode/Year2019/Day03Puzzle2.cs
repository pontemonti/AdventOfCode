...
namespace Pontemonti.AdventOfCode.Year2019
{
    ...
    /// 8+5+5+2 = 20 steps by the first wire and 7+6+4+3 = 20 steps by the second wire 
    ...
    /// </summary>
    public class Day03Puzzle2 : IPuzzle
    {
        public void Solve()
        {
            int result = CalculateResult();
            Console.WriteLine($"The fewest combined steps the wires must take to reach an intersection: {result}");
        }

        public static int CalculateResult()
        {
            Console.WriteLine("Parsing input...");
            (IEnumerable<WirePath> wire1Paths, IEnumerable<WirePath> wire2Paths) = InputHelper.ReadWirePathLines(Day03Puzzle1.input);
            Console.WriteLine("Calculating...");
            int result = GeometryHelper.CalculateStepsToClosestIntersection(wire1Paths, wire2Paths);
            return result;
        }
    }
}
